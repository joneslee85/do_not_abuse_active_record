...
describe ClaimEligibilities::TRV do
  let!(:client) { double('client') }

  describe '#eligible?' do
    it %{ triggers following eligibility rules:
      * check for policy expiration
      * check if the destination is covered
    } do
      expect_any_instance_of(ClaimEligibilityRules::PolicyExpiration).to receive(:validate).once
      expect_any_instance_of(ClaimEligibilityRules::CoveredTravelDestinations).to receive(:validate).once
